...
async def start(client, message):
    ...
    if len(message.command) != 2:
        ...
        await m.delete()
        m=await message.reply_sticker("CAACAgQAAxkBAAJExmaR-puA7Y01iHE-CMR_h0JDgxLrAAJuDQACFJTBUDDFylmdBowhHgQ") 
        await asyncio.sleep(1)
        await m.delete()
    ...
    btn = [[
        InlineKeyboardButton("♻️ Get File Again", callback_data=f'delfile#{file_id}')
    ]]
    k = await msg.reply("<b><u>💢 Files will be deleted in 10 minutes to avoid copyright issues ‼️. Please forward and save them ♻️.</i></b>",quote=True)
    await asyncio.sleep(600)
    await msg.delete()
